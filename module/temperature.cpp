  ...
  void Temperature::singlenozzle_change(const uint8_t old_tool, const uint8_t new_tool) {
      ...
      if (singlenozzle_temp[new_tool] && singlenozzle_temp[new_tool] != singlenozzle_temp[old_tool]) {
        ...
        //set_heating_message(0);
          ...
          set_heating_message(0);
          //TERN_(HAS_STATUS_MESSAGE, set_heating_message(0)); //changed
        #endif
    ...
    bool Temperature::wait_for_hotend(const uint8_t target_extruder, const bool no_wait_for_cooling/*=true*/
      ...
      void Temperature::wait_for_hotend_heating(const uint8_t target_extruder) {
        if (isHeatingHotend(target_extruder)) {
          ...
          #if ENABLED(Heating_Messages)
            SERIAL_ECHOLNPGM("Wait for hotend heating..."); //changed
            LCD_MESSAGEPGM(MSG_HEATING); //changed 
          #endif
    ...
    void Temperature::wait_for_bed_heating() {
      if (isHeatingBed()) {
        ...
        //LCD_MESSAGEPGM(MSG_BED_HEATING);
          //#if ENABLED(Heating_started_notify)
            //host_action_notify("Started Heating Bed");
          //#endif
        #if ENABLED(Heating_Messages)
          SERIAL_ECHOLNPGM("Wait for bed heating..."); // changed 
          LCD_MESSAGEPGM(MSG_BED_HEATING); // changed 
        #endif
        wait_for_bed();
        #if DISABLED(Probing_Grid_message)
          ui.reset_status();
        #endif
      }
    }
